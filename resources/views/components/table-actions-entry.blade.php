{{--duplicate--}}
<button wire:click="$emit('onDuplicateEntry', {{$id}})" wire:loading.attr="disabled" title="Copy" class="cursor-pointer inline-table items-center px-2 py-1 text-white bg-green-600 hover:bg-green-800 rounded">
...
<div class="inline" x-data="{ open: false, working: false }" x-cloak wire:key="delete-entry-{{ $id }}">
    ...
    <button x-on:click="open = true"  x-on:click.away="open = false" x-on:keydown.window.escape="open = false" title="Delete" class="cursor-pointer inline-table items-center px-2 py-1 text-white bg-red-600 hover:bg-red-800 rounded">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
